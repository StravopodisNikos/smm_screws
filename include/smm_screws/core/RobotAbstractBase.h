...
public:
    RobotAbstractBase() = default;

    explicit RobotAbstractBase(const std::string & base_yaml_path)
    {
    ...
    bool initializeFromYaml(const std::string & base_yaml_path)
    {
        ...
        if (!yaml_loader.loadAll()) {
            std::cerr << "[initializeFromYaml] Failed to load robot data from YAML."
                      << std::endl;
            return false;
        }
...
public:
    explicit FixedStructure(const std::string & base_yaml_path)
    : RobotAbstractBase(base_yaml_path)
    {
...
public:
    explicit Structure2Pseudos(const std::string & base_yaml_path)
    : RobotAbstractBase(base_yaml_path)
    {
        initializeFromYaml(base_yaml_path);
    }
    ...
    float get_PSEUDO_ANGLE(int index) override {
        if (index < 0 || index >= static_cast<int>(yaml_loader.pseudo_angles.size())) {
            std::cerr << "[StructureXPseudos::get_PSEUDO_ANGLE] Index "
...
public:
    explicit Structure3Pseudos(const std::string & base_yaml_path)
    : RobotAbstractBase(base_yaml_path)
    {
        initializeFromYaml(base_yaml_path);
    }
...
public:
    explicit Structure4Pseudos(const std::string & base_yaml_path)
    : RobotAbstractBase(base_yaml_path)
    {
        initializeFromYaml(base_yaml_path);
    }
