...
float ScrewsDynamics::DynamicManipulabilityIndex() {
    if (ptr2Jop == nullptr) {
        throw std::runtime_error("[DynamicManipulabilityIndex] ptr2Jop is null!");
    }

    if (ptr2MM == nullptr) {
        throw std::runtime_error("[DynamicManipulabilityIndex] ptr2MM is null!");
    }
...
Eigen::Matrix<float, 6, 6> ScrewsDynamics::setAlphamatrix(size_t i, size_t j) {
    if (i == 0) {
        ...
        throw std::runtime_error("ScrewsDynamics::setAlphamatrix: invalid index i (expected 0–2).");
    }
