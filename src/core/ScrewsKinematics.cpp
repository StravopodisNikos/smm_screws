...
ScrewsKinematics::ScrewsKinematics(RobotAbstractBase *ptr2abstract):  _ptr2abstract(ptr2abstract) {
    ...
    for (int i = 0; i < robot_params::DOF+1; ++i) {
        ptr2BodyJacobiansFrames[i] = new Eigen::Matrix<float, 6, 1>*[robot_params::DOF];
...
void ScrewsKinematics::initializeRelativeTfs() {
    std::cerr <<"[initializeRelativeTfs] DEPRECATED FOR AUTO-YAML-LOADING. This method is obsolete with YAML-based initialization.";

    // Initializes the matrices Bi ~ Ci_i1(0)
    _debug_verbosity = false;

    Bi[0] = *_ptr2abstract->gsai_ptr[0];
    if (_debug_verbosity) { printIsometryMatrix(Bi[0]); std::cout << std::endl;}
    for (size_t i = 1; i < robot_params::DOF+1; i++)
    {
        Bi[i] = extractRelativeTf(*_ptr2abstract->gsai_ptr[i], *_ptr2abstract->gsai_ptr[i-1]);
        if (_debug_verbosity) { printIsometryMatrix(Bi[i]); std::cout << std::endl;}
    }
    return;
}
...
void ScrewsKinematics::initializeRelativeTfs(Eigen::Isometry3f* B_i[robot_params::DOF+1]) {
    ...
    if (_debug_verbosity) { printIsometryMatrix(*B_i[0]); std::cout << std::endl;}
    for (size_t i = 1; i < robot_params::DOF+1; i++)
    {
        *B_i[i] = extractRelativeTf(*_ptr2abstract->gsai_ptr[i], *_ptr2abstract->gsai_ptr[i-1]);
        if (_debug_verbosity) { printIsometryMatrix(*B_i[i]); std::cout << std::endl;}
    }
    return;
}

void ScrewsKinematics::initializeLocalScrewCoordVectors(Eigen::Matrix<float, 6, 1> *i_X_i[robot_params::DOF+1]) {
    ...
    _Bi = extractRelativeTf(*_ptr2abstract->gsai_ptr[robot_params::DOF], *_ptr2abstract->gsai_ptr[robot_params::DOF-1]);
    vee(*i_X_i[robot_params::DOF]  , _Bi.matrix() );
    //if (_debug_verbosity) {  print6nMatrix(i_X_i, robot_params::DOF+1); std::cout << std::endl;} 
    return;
}
...
void ScrewsKinematics::initializeLocalScrewCoordVectors() {
    ...
    // For the tool frame, the relative tf must be extracted
    _Bi = extractRelativeTf(*_ptr2abstract->gsai_ptr[robot_params::DOF], *_ptr2abstract->gsai_ptr[robot_params::DOF-1]);
    vee(iXi[robot_params::DOF]  , _Bi.matrix() );
    return;
}
...
void ScrewsKinematics::initializeAnatomyActiveTwists() {
    ...
    if (_debug_verbosity)
    {
        print6nMatrix( _ptr2abstract->ptr2_active_twists_anat , robot_params::DOF);
    }
...
void ScrewsKinematics::extractPassiveTfs(Eigen::Isometry3f* passive_expos[robot_params::METALINKS]) {
...
void ScrewsKinematics::extractActiveTfs(float *q, Eigen::Isometry3f* active_expos[robot_params::DOF]) {
...
void ScrewsKinematics::extractActiveTfsAnat(float *q, Eigen::Isometry3f* active_expos[robot_params::DOF]) {
...
void ScrewsKinematics::ForwardKinematicsTCP(float *q) {
    ...
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematicsTCP] gst_x: %f", _gst(0,3));
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematicsTCP] gst_y: %f", _gst(1,3));
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematicsTCP] gst_z: %f", _gst(2,3));
    return;
}
...
void ScrewsKinematics::ForwardKinematics3DOF_1(float *q, Eigen::Isometry3f* gs_a_i[robot_params::DOF+1]) {
    ...
    *gs_a_i[0] = *(_ptr2abstract->gsai_test_ptr[0]) *  twistExp(_X, _joint_pos[0]);
    ...
    DEBUG_PRINTF(_debug_verbosity,"gs1_x: %f", _trans_vector.x());
    DEBUG_PRINTF(_debug_verbosity,"gs1_y: %f", _trans_vector.y());
    DEBUG_PRINTF(_debug_verbosity,"gs1_z: %f", _trans_vector.z()); 
    ...
    *gs_a_i[1] = *gs_a_i[0] * _Bi * twistExp(_X, q[1]);
    ...
    DEBUG_PRINTF(_debug_verbosity,"gs2_x: %f", _trans_vector.x());
    DEBUG_PRINTF(_debug_verbosity,"gs2_y: %f", _trans_vector.y());
    DEBUG_PRINTF(_debug_verbosity,"gs2_z: %f", _trans_vector.z()); 
    ...
    *gs_a_i[2] =  *gs_a_i[1] * _Bi * twistExp(_X, q[2]);
    ...
    DEBUG_PRINTF(_debug_verbosity,"gs3_x: %f", _trans_vector.x());
    DEBUG_PRINTF(_debug_verbosity,"gs3_y: %f", _trans_vector.y());
    DEBUG_PRINTF(_debug_verbosity,"gs3_z: %f", _trans_vector.z()); 
    ...
    *gs_a_i[3] =  *gs_a_i[2] * _Bi;  // this must be equal to _gst /in ForwardKinematicsTCP()
    ...
    DEBUG_PRINTF(_debug_verbosity,"gst_x_1: %f", _trans_vector.x());
    DEBUG_PRINTF(_debug_verbosity,"gst_y_1: %f", _trans_vector.y());
    DEBUG_PRINTF(_debug_verbosity,"gst_z_1: %f", _trans_vector.z()); 
...
void ScrewsKinematics::ForwardKinematics3DOF_2(float *q, Eigen::Isometry3f* gs_a_i[robot_params::DOF+1]) {
    // Returns a matrix of pointers of the robot's active joints' frames
    ...
    *gs_a_i[0] = _active_expos[0] * *(_ptr2abstract->gsai_ptr[0]) ;
    _trans_vector = gs_a_i[0]->translation();
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematics3DOF_2] gs1_x: %f", _trans_vector.x());
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematics3DOF_2] gs1_y: %f", _trans_vector.y());
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematics3DOF_2] gs1_z: %f", _trans_vector.z()); 
    ...
    *gs_a_i[1] = _active_expos[0] * _Pi[0] * _active_expos[1] * *(_ptr2abstract->gsai_ptr[1]) ;
    _trans_vector = gs_a_i[1]->translation();
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematics3DOF_2] gs2_x: %f", _trans_vector.x());
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematics3DOF_2] gs2_y: %f", _trans_vector.y());
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematics3DOF_2] gs2_z: %f", _trans_vector.z());    
    ...
    *gs_a_i[2] = _active_expos[0] * _Pi[0] * _active_expos[1] * _Pi[1] *_active_expos[2] * *(_ptr2abstract->gsai_ptr[2]) ;    
    _trans_vector = gs_a_i[2]->translation();
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematics3DOF_2] gs3_x: %f", _trans_vector.x());
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematics3DOF_2] gs3_y: %f", _trans_vector.y());
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematics3DOF_2] gs3_z: %f", _trans_vector.z()); 
    ...
    *gs_a_i[3] = _active_expos[0] * _Pi[0] * _active_expos[1] * _Pi[1] * _active_expos[2] * *(_ptr2abstract->gsai_ptr[3]) ;    
    _trans_vector = gs_a_i[3]->translation();
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematics3DOF_2] gst_x_2: %f", _trans_vector.x());
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematics3DOF_2] gst_y_2: %f", _trans_vector.y());
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematics3DOF_2] gst_z_2: %f", _trans_vector.z()); 
...
void ScrewsKinematics::ForwardKinematicsComFrames3DOF_2(float *q, Eigen::Isometry3f* gs_l_i[robot_params::DOF]) {
    ...
    *gs_l_i[0] = _active_expos_anat[0] * *(_ptr2abstract->gsli_test_ptr[0]) ;
    ...
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematicsComFrames3DOF_2] gsl1_x: %f", _trans_vector.x());
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematicsComFrames3DOF_2] gsl1_y: %f", _trans_vector.y());
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematicsComFrames3DOF_2] gsl1_z: %f", _trans_vector.z()); 
    ...
    *gs_l_i[1] = _active_expos_anat[0] * _active_expos_anat[1] * *(_ptr2abstract->gsli_test_ptr[1]) ;
    ...
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematicsComFrames3DOF_2] gsl2_x: %f", _trans_vector.x());
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematicsComFrames3DOF_2] gsl2_y: %f", _trans_vector.y());
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematicsComFrames3DOF_2] gsl2_z: %f", _trans_vector.z());    
    ...
    *gs_l_i[2] = _active_expos_anat[0] * _active_expos_anat[1] * _active_expos_anat[2] * *(_ptr2abstract->gsli_test_ptr[2]) ;
    ...
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematicsComFrames3DOF_2] gsl3_x: %f", _trans_vector.x());
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematicsComFrames3DOF_2] gsl3_y: %f", _trans_vector.y());
    DEBUG_PRINTF(_debug_verbosity,"[ForwardKinematicsComFrames3DOF_2] gsl3_z: %f", _trans_vector.z()); 
...
void ScrewsKinematics::SpatialJacobian_Tool_1(Eigen::Matrix<float, 6, 1> *Jsp_t_1[robot_params::DOF]) {
    ...
    if (_debug_verbosity) {
        std::cout << "[ScrewsKinematics] Spatial Jacobian 1:\n";
        print6nMatrix(Jsp_t_1, robot_params::DOF);
    }
}
...
void ScrewsKinematics::SpatialJacobian_Tool_1() {
    ...
    if (_debug_verbosity) { std::cout << "[ScrewsKinematics] Spatial Jacobian 1:\n"; print6nMatrix(ptr2Jsp1, robot_params::DOF);}
}

void ScrewsKinematics::SpatialJacobian_Tool_2(Eigen::Matrix<float, 6, 1> *Jsp_t_2[robot_params::DOF]) {
    ...
    if (_debug_verbosity) {  std::cout << "[ScrewsKinematics] Spatial Jacobian 2:\n";  print6nMatrix(Jsp_t_2, robot_params::DOF);}
}
...
void ScrewsKinematics::SpatialJacobian_Tool_2() {
    ...
    if (_debug_verbosity) {  std::cout << "[ScrewsKinematics] Spatial Jacobian 2:\n";  print6nMatrix(ptr2Jsp2, robot_params::DOF);}
}

void ScrewsKinematics::BodyJacobians(Eigen::Matrix<float, 6, 1>** BodyJacobiansFrames[robot_params::DOF+1]) {
    ...
    _debug_verbosity = false;
    Eigen::Matrix<float, 6, 1>* Jbd_i[robot_params::DOF]; // array of pointers to access a single(!) 6x3 Body Jacobian
    // Allocate the memory for the above pointers!

    for (size_t nFrames = 0; nFrames < robot_params::DOF+1; nFrames++)
    {
        for (size_t i = 0; i < robot_params::DOF; i++) {
        ...
        if (_debug_verbosity) {  std::cout << "[ScrewsKinematics] Body Frame Jacobian 1:\n";  print6nMatrix(Jbd_i, robot_params::DOF);}
        for (size_t i = 0; i < robot_params::DOF; i++) {
...
void ScrewsKinematics::BodyCOMJacobians() {
    // [25-7-24] Implements functions: calculateLinkCoMJacobians_3robot_params::DOF located in:
    //           ~/matlab_ws/screw_dynamics/calculateLinkCoMJacobians_3robot_params::DOF.m @ laptop-WIN10
    ...
    // Fill Jbsli by col: [Eigen::Matrix<float, 6, 1> Jbsli[robot_params::DOF][robot_params::DOF];]
    for (int m = 0; m < robot_params::DOF; ++m) {
        try {
            if (m==0) {
    ...
    // Link the pointers to Jbsli: [Eigen::Matrix<float, 6, 1>* ptr_Jbsli[robot_params::DOF][robot_params::DOF];]
    for (int m = 0; m < robot_params::DOF; ++m) {
        for (int n = 0; n < robot_params::DOF; ++n) {
    ...
    for (int m = 0; m < robot_params::DOF; ++m) {
        for (int n = 0; n < robot_params::DOF; ++n) {
            Jbsli63[m].col(n) = Jbsli[m][n];      // Fill the a6x3 arrays: [Eigen::Matrix<float, 6, robot_params::DOF> Jbsli63[robot_params::DOF];]
            //ptr_Jbsli63[m]->col(n) = Jbsli[m][n]; // Link the pointers to 6x3 arrays: [Eigen::Matrix<float, 6, robot_params::DOF>* ptr_Jbsli63[robot_params::DOF];]
        }
    }
...
void ScrewsKinematics::BodyJacobian_Tool_1(Eigen::Matrix<float, 6, 1> *Jbd_t_1[robot_params::DOF]) {
    ...
    _debug_verbosity = false;
    for (size_t i = 0; i < robot_params::DOF; i++){
        ad(_ad, g[robot_params::DOF].inverse() * g[i] );
        *Jbd_t_1[i] = _ad * iXi[i];
    }
    if (_debug_verbosity) {  std::cout << "[ScrewsKinematics] Body Jacobian Tool 1:\n"; print6nMatrix(Jbd_t_1, robot_params::DOF);}
}
...
void ScrewsKinematics::BodyJacobian_Tool_1() {
    ...
    if (_debug_verbosity) {  std::cout << "[ScrewsKinematics] Body Jacobian Tool 1:\n"; print6nMatrix(ptr2Jbd_t_1, robot_params::DOF);}
}

void ScrewsKinematics::BodyJacobian_Tool_2(Eigen::Matrix<float, 6, 1> *Jbd_t_2[robot_params::DOF]) {
...
void ScrewsKinematics::ToolVelocityTwist(typ_jacobian jacob_selection, float *dq, Eigen::Matrix<float, 6, 1> &Vtwist ) {
    ...
    case typ_jacobian::SPATIAL :
        ...
        if (_debug_verbosity) {  std::cout << "[ScrewsKinematics/ToolVelocityTwist] Spatial Velocity Twist:\n"; printTwist( Vtwist);}
        break;
    case typ_jacobian::BODY :
        // Concatenate the Spatial Jacobian column vectors to a single array:
        Jbd63 = mergeColumns2Matrix63(Jbd_t_1);    
        Vtwist = Jbd63 * dq_vector;    
        if (_debug_verbosity) {  std::cout << "[ScrewsKinematics/ToolVelocityTwist] Body Velocity Twist:\n"; printTwist( Vtwist);}
        break;    
    default:
...
void ScrewsKinematics::DtBodyJacobian_Tool_1( float *dq, Eigen::Matrix<float, 6, 1>** BodyJacobiansFrames[robot_params::DOF+1], Eigen::Matrix<float, 6, 1> *dJbd_t_1[robot_params::DOF]) {
    ...
    {
        dJ.setZero(); 
        for (size_t k = j+1; k < robot_params::DOF; k++)
        {
            dJ = dJ + lb(*BodyJacobiansFrames[robot_params::DOF][j], *BodyJacobiansFrames[robot_params::DOF][k]) * dq[k] ;
        }
        *dJbd_t_1[j] = dJ;
    }
    if (_debug_verbosity) {  std::cout << "[ScrewsKinematics/DtBodyJacobian_Tool_1] Time Derivative Body Jacobian Tool 1:\n"; print6nMatrix(dJbd_t_1, robot_params::DOF);}
    return;
}
...
void ScrewsKinematics::DtBodyJacobian_Tool_1() {
    ...
    for (size_t j = 0; j < robot_params::DOF; j++)
    {
        ...
        for (size_t k = j+1; k < robot_params::DOF; k++)
        {
            dJ = dJ + lb(*ptr2BodyJacobiansFrames[robot_params::DOF][j], *ptr2BodyJacobiansFrames[robot_params::DOF][k]) * _joint_vel[k] ;
        }
        *ptr2dJbd_t_1[j] = dJ;
    }
    if (_debug_verbosity) {  std::cout << "[ScrewsKinematics/DtBodyJacobian_Tool_1] Time Derivative Body Jacobian Tool 1:\n"; print6nMatrix(ptr2dJbd_t_1, robot_params::DOF);}
    return;
}

void ScrewsKinematics::DtBodyJacobian_Tool_2( float *dq, Eigen::Matrix<float, 6, 1>** BodyJacobiansFrames[robot_params::DOF+1], Eigen::Matrix<float, 6, 1> *dJbd_t_2[robot_params::DOF]) {
    ...
    for (size_t j = 0; j < robot_params::DOF; j++)
    {
        ...
        for (size_t k = j+1; k < robot_params::DOF; k++)
        {
            ad(_ad, g[robot_params::DOF].inverse() * g[k]);
    ...
    if (_debug_verbosity) {  std::cout << "[ScrewsKinematics/DtBodyJacobian_Tool_2] Time Derivative Body Jacobian Tool 2:\n"; print6nMatrix(dJbd_t_2, robot_params::DOF);}
    return;
}
...
void ScrewsKinematics::DtBodyJacobian_Tool_2() {
    ...
    if (_debug_verbosity) {  std::cout << "[ScrewsKinematics/DtBodyJacobian_Tool_2] Time Derivative Body Jacobian Tool 2:\n"; print6nMatrix(ptr2dJbd_t_2, robot_params::DOF);}
    return;
}

void ScrewsKinematics::OperationalSpaceJacobian(Eigen::Matrix3f &Jop_t) {
    // Returns the Operational Space Jacobian Matrix for 3robot_params::DOF serial manipulator. Links
    // cartesian velocity with joints velocities. Since 3robot_params::DOF, no rotational part is co-
    ...
    Jop_t = g[robot_params::DOF].rotation() * Jbd_pos;
    if (_debug_verbosity)
    {
        std::cout << "Operational Space Jacobian:\n"
                  << Jop_t << std::endl;
    }
    ptr2Jop = &Jop_t;
    return; 
...
Eigen::Matrix3f ScrewsKinematics::OperationalSpaceJacobian(const Eigen::Vector3f& qs) {
    ...
    //           D:\matlab_ws\Kinematic_Model_Assembly_SMM\calculateFunctions\calculateToolJacobian_3robot_params::DOF.m
...
void ScrewsKinematics::OperationalSpaceJacobian() {
    ...
    _debug_verbosity = false;
    setBodyPositionJacobian();
    Jop = g[robot_params::DOF].rotation() * Jbd_pos;
    ptr2Jop = &Jop;
    if (_debug_verbosity)
    {
        std::cout << "Operational Space Jacobian:\n"
...
void ScrewsKinematics::inverseOperationalSpaceJacobian() {
    // Returns the inverse of the Operational Space Jacobian Matrix for 3robot_params::DOF serial manipulator
    ...
    ptr2iJop = &iJop;
    if (_debug_verbosity) {
        std::cout << "Inverse Operational Space Jacobian:\n"
                  << iJop << std::endl;
    }
    return; 
}
...
Eigen::Matrix3f ScrewsKinematics::OperationalSpaceJacobian2() {
    ...
    //           Only applies to 3robot_params::DOF robot!
...
void ScrewsKinematics::DtOperationalSpaceJacobian(Eigen::Matrix3f &dJop_t) {
    ...
    dJop_t = (dRst *  Jbd_pos) + ( g[robot_params::DOF].rotation() * dJbd_pos );
    ptr2dJop = &dJop_t;
    return; 
}
...
void ScrewsKinematics::DtOperationalSpaceJacobian() {
    ...
    dJop = (dRst *  Jbd_pos) + ( g[robot_params::DOF].rotation() * dJbd_pos );
    ptr2dJop = &dJop;
    if (_debug_verbosity) {
        std::cout << "Time Derivative of Operational Space Jacobian:\n"
                  << dJop << std::endl;
    }
    return; 
}
...
void ScrewsKinematics::DtToolVelocityTwist(typ_jacobian jacob_selection) {
    ...
    case typ_jacobian::SPATIAL :
        ...
        {
            for (size_t j = k+1; j < robot_params::DOF; j++)
            {
                dV2 = dV2 + lb(Jsp_t_1[k], Jsp_t_1[j]) * _joint_vel[j] * _joint_vel[k];
            }
        }
        dVsp_tool_twist = dV1 + dV2;
        if (_debug_verbosity) {  std::cout << " [ScrewsKinematics/DtToolVelocityTwist] Spatial Acceleration Twist:\n"; printTwist(dVsp_tool_twist);}
        break;
    case typ_jacobian::BODY :
        dq_vector << _joint_vel[0], _joint_vel[1], _joint_vel[2];
        ...
        if (_debug_verbosity) {  std::cout << " Body Acceleration Twist:\n"; printTwist(dVbd_tool_twist);}
        break;    
    default:
        DEBUG_STREAM(true,"[ScrewsKinematics/DtToolVelocityTwist] WRONG JACOBIAN SELECTION FOR ACCELEARATION TWIST");
        break;
    }    
    return;
}
...
void ScrewsKinematics::DtToolVelocityTwist(typ_jacobian jacob_selection, float *ddq, float *dq, Eigen::Matrix<float, 6, 1> &dVtwist ) {
    ...
    case typ_jacobian::SPATIAL :
        ...
        dVtwist = dV1 + dV2;
        if (_debug_verbosity) {  std::cout << " Spatial Acceleration Twist:\n"; printTwist(dVtwist);}
        break;
    case typ_jacobian::BODY :
        dq_vector << dq[0], dq[1], dq[2];
        ...
        break;    
    default:
        DEBUG_STREAM(true,"WRONG JACOBIAN SELECTION FOR ACCELEARATION TWIST");
        break;
    }    
    return;
}
...
void ScrewsKinematics::CartesianVelocity_twist(Eigen::Vector4f &v_qs) {
    ...
    // Spatial velocity twist and forward kinematics tf of {T} at 
    // current configuration must be previously extracted.
    _debug_verbosity = false;
    formTwist(_twist_se3, Vsp_tool_twist);

    Eigen::Vector4f point;
    point << g[3](0, 3), g[3](1, 3), g[3](2, 3), 1.0f;

    v_qs = _twist_se3 * point;

    if (_debug_verbosity) {
        std::cout << "[CartesianVelocity_twist] v_qs = ["
                  << v_qs[0] << ", " << v_qs[1] << ", " << v_qs[2] << "]"
                  << std::endl;
    }
}
...
void ScrewsKinematics::CartesianVelocity_jacob(Eigen::Vector3f &v_qs) {
    ...
    v_qs = Jop * dq_vector;
    if (_debug_verbosity) {
        std::cout << "[CartesianVelocity_jacob] v_qs = ["
                  << v_qs[0] << ", " << v_qs[1] << ", " << v_qs[2] << "]"
                  << std::endl;
    }
    return;
...
void ScrewsKinematics::CartesianAcceleration_twist(Eigen::Vector4f &a_qs, Eigen::Vector4f v_qs ) {
    ...
    if (_debug_verbosity) {
        std::cout << "[CartesianAcceleration_twist] a_qs = ["
                  << a_qs[0] << ", " << a_qs[1] << ", " << a_qs[2] << "]"
                  << std::endl;
    }
    return;
}
...
void ScrewsKinematics::CartesianAcceleration_jacob(Eigen::Vector3f &a_qs) {
    ...
    if (_debug_verbosity) {
        std::cout << "[CartesianAcceleration_jacob] a_qs = ["
                  << a_qs[0] << ", " << a_qs[1] << ", " << a_qs[2] << "]"
                  << std::endl;
    }
    return;
...
void ScrewsKinematics::setBodyPositionJacobian() {
    ...
    Eigen::Matrix<float, 3, 1> Jpos_col[robot_params::DOF];
    for (size_t i = 0; i < robot_params::DOF; i++)
    {
    ...
    if (_debug_verbosity) {
        std::cout <<  "[setBodyPositionJacobian]\n" << std::endl;
        for (int i = 0; i < robot_params::DOF; ++i) {
            std::cout << "Jbd_pos[" << i << "]:\n" << Jpos_col[i] << std::endl;
        }
    }
...
void ScrewsKinematics::setDtBodyPositionJacobian() {
    ...
    Eigen::Matrix<float, 3, 1> dJpos_col[robot_params::DOF];
    for (size_t i = 0; i < robot_params::DOF; i++)
    {
    ...
    if (_debug_verbosity) {
        std::cout <<  "[setDtBodyPositionJacobian]\n" << std::endl;
        for (int i = 0; i < robot_params::DOF; ++i) {
...
void ScrewsKinematics::printIsometryMatrix(const Eigen::Isometry3f& matrix) {
    ...
    std::cout << std::endl; // optional extra newline
}

void ScrewsKinematics::print6nMatrix(Eigen::Matrix<float, 6, 1>* matrices[], std::size_t n) {
...
void ScrewsKinematics::print63MatrixByColumn(const Eigen::Matrix<float, 6, robot_params::DOF> J63[robot_params::DOF]) {
    for (int i = 0; i < robot_params::DOF; ++i) {
        std::cout << "J63[" << i << "]:" << std::endl;
        for (int row = 0; row < 6; ++row) {
            for (int col = 0; col < robot_params::DOF; ++col) {
