...
/*
 *  [26-7-24] Copied from idosc_centralized.cpp
 *
 ...
 * 
 *  KEY UPDATES:
 *  -> works with smm structures (see example_dynamics.cpp on how to define smm structure)
 *  -> controls a smm in gazebo using smm_ros_gazebo_xacro
 *  -> works with the fully debugged kinematic and dynamics
 *  -> fixed naming in topics for better understanding
 * 
 *  [27-7-2024] DEPRECATED after reading the essential for custom controller
 *              coding for ros_control. ALL custom operational space controllers
 *              for SMM are located in smm_control package.
 */
...
{
    ros::init(argc, argv, "smm_idosc_centralized");
    ros::console::set_logger_level(ROSCONSOLE_DEFAULT_NAME, ros::console::levels::Debug);
    ros::NodeHandle nh;

    // 1. Initialize the robot structure
    ROS_INFO("[smm_idosc_centralized] Initializing robot structure");
    ...
    ROS_INFO("[smm_idosc_centralized] Initializing shared library");
    robot_shared my_shared_lib(robot_ptr, nh);
    if (!my_shared_lib.initializeSharedLib()) {
        ROS_ERROR("[smm_idosc_centralized] Failed to initialize shared library.");
        return -1;
    }
    ROS_INFO("[smm_idosc_centralized] Initialized Shared Library.");
    ...
    desired_state(3) = 0.10f;
    desired_state(4) = 0.0f;
    desired_state(5) = 0.00f; 
    ...
    ROS_INFO("[smm_idosc_centralized] Ready to start publishing torques...");
